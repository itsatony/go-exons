...
func TestConfig_Getters_NilReceiver(t *testing.T) {
	var c *Config
	assert.Equal(t, "", c.GetProvider())
	assert.Equal(t, "", c.GetModel())
	v, ok := c.GetTemperature()
	assert.Equal(t, float64(0), v)
	assert.False(t, ok)
	iv, iok := c.GetMaxTokens()
	assert.Equal(t, 0, iv)
	assert.False(t, iok)
	assert.Nil(t, c.GetStopSequences())
	assert.Nil(t, c.GetThinking())
	assert.Nil(t, c.GetResponseFormat())
	assert.Nil(t, c.GetGuidedDecoding())
	assert.False(t, c.HasThinking())
	assert.False(t, c.HasResponseFormat())
	assert.False(t, c.HasGuidedDecoding())
	assert.False(t, c.HasMinP())
	assert.False(t, c.HasRepetitionPenalty())
	assert.False(t, c.HasSeed())
	assert.False(t, c.HasLogprobs())
	assert.False(t, c.HasStopTokenIDs())
	assert.False(t, c.HasLogitBias())
	assert.Equal(t, "", c.GetModality())
	assert.False(t, c.HasModality())
	assert.Nil(t, c.GetImage())
	assert.False(t, c.HasImage())
	assert.Nil(t, c.GetAudio())
	assert.False(t, c.HasAudio())
	assert.Nil(t, c.GetEmbedding())
	assert.False(t, c.HasEmbedding())
	assert.Nil(t, c.GetStreaming())
	assert.False(t, c.HasStreaming())
	assert.Nil(t, c.GetAsync())
	assert.False(t, c.HasAsync())
	assert.False(t, c.HasFrequencyPenalty())
	assert.False(t, c.HasPresencePenalty())
	assert.False(t, c.HasN())
	assert.False(t, c.HasMaxCompletionTokens())
	assert.Equal(t, "", c.GetReasoningEffort())
	assert.False(t, c.HasReasoningEffort())
	assert.False(t, c.HasTopA())
	assert.Equal(t, "", c.GetUser())
	assert.False(t, c.HasUser())
	assert.Equal(t, "", c.GetServiceTier())
	assert.False(t, c.HasServiceTier())
	assert.False(t, c.HasStore())
}

func TestConfig_Getters_SetValues(t *testing.T) {
	c := &Config{
		Provider:            "openai",
		Model:               "gpt-4",
		Temperature:         ptrFloat64(0.7),
		...
		GuidedDecoding:      &GuidedDecoding{Backend: GuidedBackendAuto},
		Image:               &ImageConfig{Width: ptrInt(1024)},
	...
	assert.Equal(t, "openai", c.GetProvider())
	assert.Equal(t, "gpt-4", c.GetModel())

	temp, tok := c.GetTemperature()
	assert.Equal(t, 0.7, temp)
	assert.True(t, tok)

	mt, mtok := c.GetMaxTokens()
	assert.Equal(t, 1000, mt)
	assert.True(t, mtok)

	tp, tpok := c.GetTopP()
	assert.Equal(t, 0.9, tp)
	assert.True(t, tpok)

	tk, tkok := c.GetTopK()
	assert.Equal(t, 40, tk)
	assert.True(t, tkok)

	assert.Equal(t, []string{"stop"}, c.GetStopSequences())

	assert.True(t, c.HasMinP())
	mp, mpok := c.GetMinP()
	assert.Equal(t, 0.1, mp)
	assert.True(t, mpok)

	assert.True(t, c.HasRepetitionPenalty())
	rp, rpok := c.GetRepetitionPenalty()
	assert.Equal(t, 1.2, rp)
	assert.True(t, rpok)

	assert.True(t, c.HasSeed())
	sd, sdok := c.GetSeed()
	assert.Equal(t, 42, sd)
	assert.True(t, sdok)

	assert.True(t, c.HasLogprobs())
	lp, lpok := c.GetLogprobs()
	assert.Equal(t, 5, lp)
	assert.True(t, lpok)

	assert.True(t, c.HasStopTokenIDs())
	assert.Equal(t, []int{100}, c.GetStopTokenIDs())

	assert.True(t, c.HasLogitBias())
	assert.Equal(t, map[string]float64{"0": 1.0}, c.GetLogitBias())

	assert.True(t, c.HasFrequencyPenalty())
	fp, fpok := c.GetFrequencyPenalty()
	assert.Equal(t, 0.5, fp)
	assert.True(t, fpok)

	assert.True(t, c.HasPresencePenalty())
	pp, ppok := c.GetPresencePenalty()
	assert.Equal(t, 0.3, pp)
	assert.True(t, ppok)

	assert.True(t, c.HasN())
	n, nok := c.GetN()
	assert.Equal(t, 2, n)
	assert.True(t, nok)

	assert.True(t, c.HasMaxCompletionTokens())
	mct, mctok := c.GetMaxCompletionTokens()
	assert.Equal(t, 4096, mct)
	assert.True(t, mctok)

	assert.True(t, c.HasReasoningEffort())
	assert.Equal(t, ReasoningEffortHigh, c.GetReasoningEffort())

	assert.True(t, c.HasTopA())
	ta, taok := c.GetTopA()
	assert.Equal(t, 0.5, ta)
	assert.True(t, taok)

	assert.True(t, c.HasUser())
	assert.Equal(t, "user-123", c.GetUser())

	assert.True(t, c.HasServiceTier())
	assert.Equal(t, ServiceTierAuto, c.GetServiceTier())

	assert.True(t, c.HasStore())
	st, stok := c.GetStore()
	assert.True(t, st)
	assert.True(t, stok)

	assert.True(t, c.HasModality())
	assert.Equal(t, ModalityText, c.GetModality())

	assert.True(t, c.HasThinking())
	assert.NotNil(t, c.GetThinking())

	assert.True(t, c.HasResponseFormat())
	assert.NotNil(t, c.GetResponseFormat())

	assert.True(t, c.HasGuidedDecoding())
	assert.NotNil(t, c.GetGuidedDecoding())

	assert.True(t, c.HasImage())
	assert.NotNil(t, c.GetImage())

	assert.True(t, c.HasAudio())
	assert.NotNil(t, c.GetAudio())

	assert.True(t, c.HasEmbedding())
	assert.NotNil(t, c.GetEmbedding())

	assert.True(t, c.HasStreaming())
	assert.NotNil(t, c.GetStreaming())

	assert.True(t, c.HasAsync())
	assert.NotNil(t, c.GetAsync())
}
